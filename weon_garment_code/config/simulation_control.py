...
class SimulationControl(BaseModel):
    ...
    sim_substeps: int = Field(
        default=10, description="Number of substeps per simulation frame"
    )
    sim_wo_gravity_percentage: float = Field(
        default=0.0, description="Percentage of simulation without gravity (0-100)"
    )
    zero_gravity_steps: int = Field(
        default=5, description="Number of initial steps to run without gravity"
    )
    resolution_scale: float = Field(
        default=1.0, description="Resolution scaling factor for mesh generation"
    )
    ground: bool = Field(
        default=True, description="Whether to include a ground plane in the simulation"
    )

    # Stopping criteria
    static_threshold: float = Field(
        default=0.01,
        description="Threshold for detecting static equilibrium (L1 norm per vertex)",
    )
    transition_frame_steps: int = Field(
        default=100, description="Number of frames to interpolate between each pose"
    )
    max_sim_steps: int = Field(
        default=200, description="Maximum number of simulation steps before timeout"
    )
    max_frame_time: int | None = Field(
        default=None,
        description="Maximum time per frame in seconds (None for no limit)",
    )
    max_sim_time: int = Field(
        default=1500, description="Maximum total simulation time in seconds"
    )
    non_static_percent: float = Field(
        default=5.0,
        description="Percentage of vertices that can be non-static before considering static",
    )

    # Quality filters
    max_body_collisions: int = Field(
        default=0,
        description="Maximum allowed body-cloth collisions before quality failure",
    )
    max_self_collisions: int = Field(
        default=0,
        description="Maximum allowed self-intersections before quality failure",
    )

    # Computed field (will be set by update_min_steps)
    min_sim_steps: int = Field(
        default=0,
        description="Minimum number of simulation steps required (computed from options)",
    )
    ...
    def update_min_steps(self, options: Any) -> None:
        ...
        if options.enable_body_smoothing:
            min_steps = (
                options.smoothing_recover_start_frame + options.smoothing_num_steps
            )
