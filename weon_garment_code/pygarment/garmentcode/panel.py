...
from weon_garment_code.pygarment.garmentcode.edge import CircleEdge, EdgeSequence
from weon_garment_code.pygarment.garmentcode.interface import Interface
from weon_garment_code.pygarment.garmentcode.operators import cut_into_edge
from weon_garment_code.pygarment.garmentcode.utils import close_enough, vector_align_3D
...
class Panel(BaseComponent):
    ...
    label: str  # Additional panel label (non-unique)
    translation: np.ndarray  # 3D translation vector
    rotation: R  # 3D rotation as scipy Rotation object
    edges: EdgeSequence  # Edge sequence defining the panel shape
    ...
    def __init__(self, name, label="") -> None:
        ...
        * label: additional panel label (non-unique)
        ...
        self.rotation = R.from_euler("XYZ", [0, 0, 0])
        self.symmetry_partner: str = ""
        # zero rotation
    ...
    def top_center_pivot(self):
        ...
        mid_points_2D = [[mid_x, top_right[1]], [mid_x, low_left[1]], [top_right[0], mid_y], [low_left[0], mid_y]]
        mid_points_3D = np.vstack(tuple([self.point_to_3D(coords) for coords in mid_points_2D]))
    ...
    def add_dart(
        ...
        * Modifies edge_sequnces with full set (edge_seq) or only the interface part (int_edge_seq)
        ...
        """
        edges_new, dart_edges, int_new = cut_into_edge(dart_shape, edge, offset=offset, right=right)

        self.stitching_rules.append((Interface(self, dart_edges[0]), Interface(self, dart_edges[1])))
    ...
    def assembly(self):
        ...
        for i in range(len(self.edges)):
            ...
            self.edges[
                i
            ].geometric_id = edge_shift  # remember the mapping of logical edge to geometric id in panel loop
        ...
        panel_dict = vars(panel)  # Convert Namespace to dict for modification
        if self.symmetry_partner:
            panel_dict["symmetry_partner"] = self.symmetry_partner
        spattern.pattern["panels"] = {self.name: panel_dict}
    ...
    def norm(self):
        ...
        if close_enough(np.linalg.norm(avg_norm), 0):
            ...
            avg_norm = norms[0]
            if self.verbose:
                logger.warning("Norm evaluation failed, assigning norm based on the first edge")
    ...
    def save_as_image(self, png_filename, svg_filename=None, with_text=True, view_ids=True, margin=2):
        ...
        # Save as image
        vis_pattern._save_as_image(svg_filename, png_filename, with_text=with_text, view_ids=view_ids, margin=margin)
