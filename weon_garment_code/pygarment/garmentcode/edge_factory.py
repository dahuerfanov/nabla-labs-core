...
class CircleEdgeFactory:
    ...
    def from_rad_length(rad, length, right=True, start=None):
        ...
        if length > max_len:
            raise ValueError("CircleEdge::ERROR::Incorrect length for specified radius")
...
class CurveEdgeFactory:
    ...
    def curve_from_tangents(
        ...
        bounds = [
            (0, 1),
            (-2.0, 2.0),
        ]  # Allow some extrapolation in x, reasonable y range

        # Use L-BFGS-B for bounded optimization, or BFGS if no constraints
        has_constraints = (
            tan0_min_angle is not None
            or tan0_max_angle is not None
            or tan1_min_angle is not None
            or tan1_max_angle is not None
        )
        ...
        if not out.success:
            ...
            if final_obj > 1e-2:  # More lenient threshold
                logger.warning(
                    f"Optimization not successful (objective: {final_obj:.2e})"
                )
                if verbose:
                    logger.debug(f"Optimization details: {out}")
            elif verbose and not out.success:
                logger.debug(
                    f"Optimization reported failure but objective is acceptable ({final_obj:.2e}), using result"
                )
...
class EdgeSeqFactory:
        ...
        if len(edges) > 1:
            for i in range(1, len(edges)):
                if not all(
                    ...
                    for s, e in zip(edges[i].start, edges[i - 1].end, strict=True)
                ):
                    raise ValueError(
    ...
    def halfs_from_svg(svg_filepath, target_height=None):
        ...
        * target_height -- scales the shape s.t. it's height matches the given
        ...
        left_seqs = [
            p.reflect([bbox[0], center_y], [bbox[1], center_y]) for p in left_seqs
        ]
        right_seqs = [
            p.reflect([bbox[0], center_y], [bbox[1], center_y]) for p in right_seqs
        ]
        ...
        if tan0_min_angle is not None and tan0_max_angle is not None:
            ...
            if tan0_max_angle - tan0_min_angle > np.pi:
                ...
                if angle < tan0_min_angle and angle > tan0_max_angle:
                    ...
                    dist_to_min = min(
                        abs(angle - tan0_min_angle),
                        abs(angle + 2 * np.pi - tan0_min_angle),
                    )
                    dist_to_max = min(
                        abs(angle - tan0_max_angle),
                        abs(angle - 2 * np.pi - tan0_max_angle),
                    )
        ...
        if tan1_min_angle is not None and tan1_max_angle is not None:
            ...
            if tan1_max_angle - tan1_min_angle > np.pi:
                ...
                if angle < tan1_min_angle and angle > tan1_max_angle:
                    # Angle is in the "forbidden" region (wrapping case)
                    dist_to_min = min(
                        abs(angle - tan1_min_angle),
                        abs(angle + 2 * np.pi - tan1_min_angle),
                    )
                    dist_to_max = min(
                        abs(angle - tan1_max_angle),
                        abs(angle - 2 * np.pi - tan1_max_angle),
                    )
                    fin += constraint_penalty * min(dist_to_min, dist_to_max) ** 2
            else:
                # Normal case: no wrapping
...
def split_half_svg_paths(paths):
    ...
    # Mid-Intersection
    inter_segment = svgpath.Line(center_x + 1j * bbox[2], center_x + 1j * bbox[3])
    ...
    for p in paths:
        ...
        if len(intersect_t) != 2:
            raise ValueError(
                "SplitSVGHole::ERROR::Each Provided Svg path should cross vertical like exactly 2 times"
            )
