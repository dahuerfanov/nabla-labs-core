...
class StitchingRule:
    ...
    def __init__(self, int1: Interface, int2: Interface, verbose: bool = False) -> None:
        """
        Inputs:
        ...
        while in_id < len(inter.edges) and add_id < len(to_add):
            ...
            if close_enough(next_init, next_added, tol):
                ...
                if flip:
                    ...
                    if self.verbose:
                        logger.info(
                            f"{base_edge} from {base_panel.name} reoriented in interface"
                        )
                ...
                inter.substitute(
                    base_edge, subdiv, [inter.panel[in_id] for _ in range(len(subdiv))]
                )
    ...
    def assembly(self):
        ...
        if self.verbose and not self.isMatching():
            logger.warning("Stitch sides do not match on assembly")

        stitches = []

        for i, j in zip(
            range(len(self.int1.edges)), range(len(self.int2.edges)), strict=True
        ):
            stitches.append(
                [
                    {
