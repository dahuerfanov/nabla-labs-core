...
from weon_garment_code.pygarment.garmentcode.utils import close_enough
...
class Interface:
        ...
        for r in self.ruffle:
            if not close_enough(r["coeff"], 1, 1e-3):
                if (
                    ...
                    or not projected[r["sec"][1] - 1 : r["sec"][1] + 1].isChained()
                ):
    ...
    def __str__(self) -> str:
        return (
            f"Interface: {[p.name for p in self.panel]}: {str(self.oriented_edges())}"
        )
    ...
    def clone(self):
        """Create a deep copy of the interface, including its edges"""
        new_int = copy(self)
        from weon_garment_code.pygarment.garmentcode.edge import EdgeSequence

        new_int.edges = EdgeSequence(*self.edges)
        # Ensure panel list is also copied (it's small but still)
        new_int.panel = list(self.panel)
        new_int.edges_flipping = list(self.edges_flipping)
        return new_int
