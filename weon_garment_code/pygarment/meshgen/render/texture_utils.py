...
def texture_mesh_islands(
    ...
    out_fabric_tex_image_path: Path | None = None,
    out_mtl_file_path: Path | None = None,
    boundary_width=0.3,
    dpi=1200,
    background_img_path: Path | None = None,
    background_resolution: float = 1.0,
    uv_padding: int = 3,
    mat_name: str = "islands_texture",
):
    """
    ...
    uv_list, width = normalize_UVs(
        all_uvs, axis_padding=uv_padding
...
def create_UV_island_texture(
    ...
    if background_img_path is not None:
        ...
        ax.imshow(
            back_img[
                : int(width * back_crop_scale), : int(height * back_crop_scale), :
            ],
