...
def cut_corner(
    target_shape: EdgeSequence, target_interface: Interface, verbose: bool = False
):
    ...
    * target_interface -- the chained pairs of edges that form the corner
    ...
    if verbose and not out.success:
        logger.error(
            "Finding the projection (translation) is unsuccessful. Likely an error in edges choice"
        )
        logger.debug(str(out))

    if verbose and not close_enough(out.fun):
        logger.warning(f"Projection on {target_interface} finished with fun={out.fun}")
        logger.debug(str(out))
...
def cut_into_edge(
    target_shape, base_edge: Edge, offset=0, right=True, flip_target=False, tol=1e-2
):
    ...
    * Edges that lie on the original base edge
    ...
    if isinstance(target_shape, EdgeSequence):
        return cut_into_edge_single(target_shape, base_edge, offset, right, tol)
    ...
    sorted_tup = sorted(zip(per_seq_offsets, target_shape, strict=False), reverse=True)
...
def cut_into_edge_single(
    target_shape, base_edge: Edge, offset=0, right=True, tol=1e-2, verbose: bool = False
):
    """Insert edges of the target_shape into the given base_edge, starting
        from offset
    ...
    * Edges that lie on the original base edge
    ...
    new_edges = target_shape.copy().snap_to(
        [0, 0]
    ...
    if (
        ...
        raise ValueError(
            "Operators-CutingIntoEdge::ERROR::offset value is not within the base_edge length"
        )
    ...
    right_position = (
        np.sign(np.cross(insert_vector, avg_vertex - np.asarray(new_edges[0].start)))
        == -1
    )
...
def _fit_location_corner(l, diff_target, curve1, curve2, verbose: bool = False):
...
def _fit_location_edge(shift, location, width_target, curve, verbose: bool = False):
    """Find the points on two curves s.t. vector between them is the same as
    shortcut"""
    ...
    if verbose:
        logger.debug(
            f"Location Progression: {(_dist(point1, point2) - width_target) ** 2}"
        )
    ...
    * Or CurveEdge instance, if return_as_edge=True
    ...
    if not isinstance(curve, svgpath.CubicBezier):
        raise NotImplementedError(
            "Curve_match_tangents::ERROR::Only Cubic Bezier curves are supported ",
            f"(got {type(curve)})",
        )
...
def _fit_scale(s, shortcut, v1, v2, vc, d_v1, d_v2):
    ...
    shifted[0] += (shortcut[0] - shortcut[1]) * s[
        0
    ]  # this only changes the end vertex though
    shifted[1] += (shortcut[1] - shortcut[0]) * s[
        1
    ]  # this only changes the end vertex though
